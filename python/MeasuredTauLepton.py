...
class MeasuredTauLepton:
        ...
        if full_setup:
            ...
            if (self.preciseVisMass < 0.9*minVisMass or self.preciseVisMass > 1.1*maxVisMass):
                ...
                if self.type == MeasuredTauLepton.kTauToElecDecay:
                    ...
                    print(f"Invalid type {self.type} declared for leg: Pt = {self.pt}, eta = {self.eta}, phi = {self.phi}, mass = {self.mass} !!")
                    return
    ...
    def initialize(self):
        ...
        self.p4 = vector.obj(px = self.px, py = self.py, pz = self.pz, E = self.energy)
        self.p3 = vector.obj(px = self.px, py = self.py, pz = self.pz)
